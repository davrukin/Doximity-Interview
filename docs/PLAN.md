...
| 2026-06-10 | Database back to version 1; v2 schema and migration removed                       | The v2 NOT NULL schema was unshippable (see above) and its migration declared DEFAULT clauses Room's validator rejects. No release ever shipped v2; dev devices downgrade destructively. Temporarily renamed to `watchlist_v2.db` to avoid stale-file crashes during development; cleaned up once the workaround was no longer needed. |
...
| 2026-06-11 | Sparkline & row states migrated to rememberSaveable                               | Migrated to rememberSaveable with custom DoubleArray Saver to persist sparkline history. Added lastOrNull() check in LaunchedEffect to prevent double-adding snapshot price on rotation (flat lines when market closed). |
| 2026-06-11 | Added distinctUntilChanged to ObserveQuotesUseCase                                | Database writes (persisting ticks every 5s) triggered observeWatchlist emissions, causing flatMapLatest to cancel and restart the WebSocket/snapshot stream. Deduplicating by instrument list resolved the stale-flashing loop. |
