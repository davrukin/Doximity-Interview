        ...
        entryProvider =
            entryProvider {
                entry<WatchlistKey> {
                    ...
                    val params =
                        remember(backStack) {
                            WatchlistPresenter.Params(onOpenSearch = { backStack.add(SearchKey) })
                        }
                    WatchlistScreen(model = presenter.present(params = params))
                }
                entry<SearchKey> {
                    val presenter = koinInject<SearchPresenter>()
                    val params =
                        remember(backStack) {
                            SearchPresenter.Params(onBack = { backStack.removeLastOrNull() })
                        }
                    SearchScreen(model = presenter.present(params = params))
                }
            },
    )
}
